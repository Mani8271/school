...
const StudentsList = () => {
  ...
  const [sectionQuery, setSectionQuery] = useState("All Sections"); // Section filter state
 ...
 useEffect(() => {
  ...
  // If class is changed, filter section options
  if (name === "class") {
    const matchedSections = sections2
      .filter((sec) => sec.className === value)
      .map((sec) => sec.sectionName);

    setSectionOptions(["Select Section", ...new Set(matchedSections)]);
    
    // Optionally reset section if class is changed
    setNewStudent((prev) => ({
      ...prev,
      [name]: value,
  ...
  if (selected === "All Classes") {
    ...
    const filtered = sections2
      .filter((sec) => sec.className === selected)
      .map((sec) => sec.sectionName);

    const uniqueSections = ["All Sections", ...new Set(filtered)];
    setSectionOptions(uniqueSections);
  }

  // Reset section filter
  setSectionQuery("All Sections");
};
...
useEffect(() => {
  ...
  console.log(allstudents, "allstudents123");
  console.log(classFilter, "classFilter");
  console.log(sectionQuery, "sectionQuery");

  const filteredStudents2 = allstudents.filter((student) => {
    const classMatch =
      classFilter === "All Classes" || student.class === classFilter;
    const sectionMatch =
      sectionQuery === "All Sections" || student.section === sectionQuery;
  ...
  setFilteredStudents(filteredStudents2);
  console.log(filteredStudents2, "filteredStudents2");
}, [allstudents, classFilter, sectionQuery, searchQuery]);
            ...
            {[
              {
               ...
               options: ["All Classes", ...classes.map((cls) => cls.className)],
              },
              {
                value: sectionQuery,
              ...
              >
                {dropdown.options.map((option, i) => (
                  <option key={i} value={typeof option === "number" ? option : option}>
                    {option}
                  </option>
                ))}
              </select>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {[
                    ...
                    { label: "Roll Number*", name: "rollno", type: "text" },
                    {
                      label: "Class*",
                      name: "class",
                      options: ["Select Class", ...classes.map((cls) => cls.className)],
                      type: "dropdown"
                    },
      ...
      {type === "dropdown" ? (
        ...
        >
          {options.map((option, i) => (
            <option key={i} value={option === "Select Class" ? "" : option}>
              {option}
            </option>
          ))}
        </select>
