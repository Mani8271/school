...
//         { date: "2025-06-03", status: "On Leave" },
//         { date: "2025-06-04", status: "Holiday" },
//       ],
//     },
//     {
//       id: "S54346",
...
import { useNavigate } from "react-router-dom";
import { IoArrowBack } from "react-icons/io5";
import { getStudentAttendanceInitiate } from "../redux/actions/student/getmonthlyattendenceAction";
...
const MonthlyStudentAttendance = () => {
  ...
  const dispatch = useDispatch();
   const { data, loading, error } = useSelector(
      (state) => state.studentmontlyattendence.data || {}
    );
 ...
 const filteredStudent = monthlyData.filter((student) => {
  ...
  const countAttendance = (student, type) =>
    student.attendance.filter((record) => record.status === type).length;

  // Calendar tile content
  const getAttendanceForCalendar = () => {
    return viewAttendanceStudent?.attendance || [];
  };
              ...
              filteredStudent.map((student) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <button
                      className="text-blue-500 hover:underline"
                      onClick={() => setViewAttendanceStudent(student)}
                    >
                      View Attendance
                    </button>
                  </td>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-gray-500 bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-lg shadow-md w-auto max-w-3xl overflow-y-auto max-h-[80vh]">
            <h2 className="mb-4 text-2xl font-bold">
              Attendance for {viewAttendanceStudent.name}
            </h2>
            <Calendar
              tileContent={tileContent}
              className="react-calendar"
              aria-label={`Calendar view for ${viewAttendanceStudent.name}`}
            />
            <button
