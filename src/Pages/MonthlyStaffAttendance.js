...
import { getNTMonthlyAttendanceInitiate } from "../redux/actions/staff/nonteachingstaff/ntMonthlyAttendenceAction";
...
const MonthlyStaffAttendance = () => {
  ...
  const { data, loading, error } = useSelector(
    (state) => state.getnonteachingmonthlyattendence.data || {}
  );

  console.log("Monthly Attendance Data:", data);
  const monthlyData = Array.isArray(data) ? data : [];

  const { selectedBranch } = useBranch(); // Assuming branch is used to filter

  // Fetch data when month/year change
  useEffect(() => {
    if (selectedMonth && selectedYear) {
      dispatch(
        getNTMonthlyAttendanceInitiate({
          ...
          if (callback) {
            console.log("Monthly attendance fetched successfully");
          }
        })
      );
    }
  ...
  const countAttendance = (teacher, type) =>
    teacher.attendance.filter((record) => record.status === type).length;

  const getAttendanceForCalendar = () => {
    if (!selectedTeacher) return [];
    return selectedTeacher.attendance;
  };


  const tileContent = ({ date }) => {
  // Extract year, month, day from Calendar date
  ...
  const filteredStaff =  monthlyData.filter((staff) =>
    staff.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="min-h-screen p-6 bg-gray-50">
      ...
      <h1 className="mb-2 text-3xl font-bold text-gray-800">
        Monthly Teaching Staff Attendance
      </h1>
      ...
      {/* Loading / Error State */}
      {loading && <p>Loading monthly attendance...</p>}
      ...
      {!loading && !error && (
        <div className="overflow-x-auto overflow-y-auto" style={{ maxHeight: "60vh" }}>
          <table className="min-w-full bg-white rounded-lg shadow-md">
            ...
            <tbody>
              {filteredStaff.length > 0 ? (
                filteredStaff.map((teacher, index) => (
