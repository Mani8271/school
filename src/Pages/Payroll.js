...
import { AddPayrollInitiate } from "../redux/actions/payroll/addPayrollAction";
import { useDispatch, useSelector } from "react-redux";
import { getAllPayrollInitiate } from "../redux/actions/payroll/getAllpayrollAction";
import { UpdatePayrollInitiate } from "../redux/actions/payroll/updatepayrollAction";
import { DeletePayrollInitiate } from "../redux/actions/payroll/deletepayrollAction";
...
const Payroll = () => {
 ...
 const payrollState = useSelector((state) => state.getallpayroll);
const salaryData = payrollState.payroll.data || [];
  ...
  const totalPages = Math.ceil(salaryData.length / entriesCount); // Total pages based on data
   useEffect(() => {
    dispatch(getAllPayrollInitiate());
  }, [dispatch]);
 ...
 const filteredSalaryData = salaryData.filter((salary) => {
  ...
  console.log("Filtering Salary Data:", salary.month);
  if (selectedMonth) {
    return salary.month.toLowerCase().trim() === selectedMonth.toLowerCase().trim();
  }
  return true;
});
 ...
 const handleSaveSalary = async () => {
  const newSalaryData = {
    ...
    employeeId: "EMP_RANDOM", // Replace with actual employee ID if available
    netSalary,
    month: currentMonth,
    ...
    allowance: earnings.allowance,
    medicalAllowance: earnings.medicalAllowance,
    others: earnings.others,
    tds: deductions.tds,
    esi: deductions.esi,
    pf: deductions.pf,
    leaves: deductions.leaves,
    profTax: deductions.profTax,
  
    joiningDate: "2022-01-01", // Replace with real data if needed
    role: "Software Engineer",
  };

  if (editingSalaryIndex !== null) {
    // Get existing record and extract _id
    const salaryToEdit = salaryData[editingSalaryIndex];

    const updatedSalaryData = {
      ...newSalaryData,
      _id: salaryToEdit?._id, // Required for update API
    };

    dispatch(UpdatePayrollInitiate(updatedSalaryData, (success) => {
      if (success) {
        console.log("✅ Salary updated successfully");
        dispatch(getAllPayrollInitiate()); // Refresh list
        setSelectedMonth("");
        handleClose();
      } else {
    ...
    dispatch(AddPayrollInitiate(newSalaryData, (success) => {
      if (success) {
  ...
  const salary = salaryData[index];
      console.log("Editing salary record:", salary);
  setSelectedEmployee(salary.employeeName);
 ...
 const handleDeleteSalary = (index) => {
  ...
  // Get the _id of the salary record
  const salaryToDelete = salaryData[index];

  if (!salaryToDelete || !salaryToDelete._id) {
    alert("Salary record is missing an ID.");
    return;
  }

  // Dispatch the delete action
  dispatch(DeletePayrollInitiate({ _id: salaryToDelete._id }, (success) => {
    if (success) {
